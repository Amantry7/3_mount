...
async def start(message: types.Message):
    await message.answer("Здравствуйте! Я бот, который обменяет ваши деньги. Введите сколько хотите разменять")
    await Money.money.set()

@dp.message_handler(state=Money.money)
async def money(message: types.Message, state: FSMContext):
    await state.update_data(money=message.text)
    await message.answer("Выберите валюту для размена.", reply_markup=currency_keyboard)
    await Money.next()
...
async def handle_eur_callback(call: types.CallbackQuery, state: FSMContext):
